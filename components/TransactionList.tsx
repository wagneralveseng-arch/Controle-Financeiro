                 ...
                 <div key={t.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-4 group">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 text-[10px] text-slate-500 uppercase font-bold tracking-wider mt-1">
                                ...
                                <span>•</span>
                                <span className="bg-slate-200 dark:bg-slate-800 px-1 rounded text-slate-600 dark:text-slate-300">{t.category}</span>
                                {t.linkedDebtId && (
                                    <span className="flex items-center gap-1 text-red-600 dark:text-red-300 bg-red-100 dark:bg-red-900/30 px-1 rounded border border-red-200 dark:border-red-900/50">
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-6 pl-10 md:pl-0">
                        <span className={`font-bold font-mono text-base ${
                            ...
                            t.type === 'SAVING' ? 'text-blue-600 dark:text-blue-300' : 
                        ...
                        <div className="flex gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                            <button 
                                ...
                                className="p-2 bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-300 dark:hover:bg-slate-700 rounded-sm"
                                title="Editar"
                            >
                            ...
                            <button 
                                ...
                                className="p-2 bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-500 hover:bg-slate-300 dark:hover:bg-slate-700 rounded-sm"
                                title="Excluir"
                            >
      ...
      <div className="bg-white dark:bg-slate-900 p-4 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between transition-colors rounded-sm">
        <div className="flex items-center gap-4">
          <button 
            ...
            className={`px-4 py-2 text-sm font-bold uppercase tracking-wider transition-colors border border-slate-200 dark:border-slate-700 ${filterMode === 'ALL' ? 'bg-slate-100 dark:bg-slate-100 text-slate-900' : 'bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
          >
            Geral
          </button>
          <button 
            onClick={() => setFilterMode('MONTH')}
            className={`px-4 py-2 text-sm font-bold uppercase tracking-wider transition-colors border border-slate-200 dark:border-slate-700 ${filterMode === 'MONTH' ? 'bg-slate-100 dark:bg-slate-100 text-slate-900' : 'bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
          >
            Mês Específico
          </button>
        </div>
        ...
        {filterMode === 'MONTH' && (
          <div className="flex items-center gap-2">
            <select 
              ...
              className="border border-slate-200 dark:border-slate-700 p-2 text-sm font-medium bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white outline-none focus:border-slate-400 dark:focus:border-white transition-colors"
            >
            ...
            <input 
              ...
              className="border border-slate-200 dark:border-slate-700 p-2 w-20 text-sm font-medium bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white outline-none focus:border-slate-400 dark:focus:border-white transition-colors"
            />
          </div>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm p-6 transition-colors rounded-sm">
          ...
          <div className="flex flex-col md:flex-row items-center justify-around gap-8">
                    ...
                    <PieChart>
                      ...
                      >
                        {summaryStats.chartData.map((entry, index) => {
                          ...
                          else if (entry.name === 'Poupança') color = 'var(--saving-color, #94a3b8)'; // Will rely on CSS var or fallback
                          else color = '#334155';
                          
                          // Handle Theme Colors manually since Recharts is canvas/svg based
              ...
              <div className="w-full md:w-2/3 grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="bg-slate-100 dark:bg-slate-800 p-4 border-l-4 border-slate-900 dark:border-white transition-colors">
                      <div className="flex items-center gap-2 mb-1">
                          <ArrowUpCircle className="w-4 h-4 text-slate-500 dark:text-slate-400" />
                          <span className="text-xs font-bold uppercase text-slate-600 dark:text-slate-400">Receita</span>
                      </div>
                      <p className="text-lg font-bold text-slate-900 dark:text-white">R$ {summaryStats.totalIncome.toLocaleString('pt-BR')}</p>
                  </div>

                  <div className="bg-yellow-50 dark:bg-slate-800 p-4 border-l-4 border-yellow-500 transition-colors">
                      <div className="flex items-center gap-2 mb-1">
                          <ArrowDownCircle className="w-4 h-4 text-yellow-600 dark:text-yellow-500" />
                          <span className="text-xs font-bold uppercase text-slate-600 dark:text-slate-400">Despesas</span>
                      </div>
                      <p className="text-lg font-bold text-yellow-700 dark:text-yellow-500">R$ {summaryStats.totalExpense.toLocaleString('pt-BR')}</p>
                  </div>

                  <div className="bg-slate-100 dark:bg-slate-800 p-4 border-l-4 border-slate-400 transition-colors">
                      <div className="flex items-center gap-2 mb-1">
                          <PiggyBank className="w-4 h-4 text-slate-500 dark:text-slate-300" />
                          <span className="text-xs font-bold uppercase text-slate-600 dark:text-slate-400">Poupança</span>
                      </div>
                      <p className="text-lg font-bold text-slate-800 dark:text-slate-200">R$ {summaryStats.totalSavings.toLocaleString('pt-BR')}</p>
                  </div>

                  <div className={`p-4 border border-dashed text-center flex flex-col justify-center transition-colors ${summaryStats.percentCompromised > 100 ? 'bg-yellow-50 dark:bg-yellow-950/20 border-yellow-300 dark:border-yellow-800' : 'bg-slate-50 dark:bg-slate-900 border-slate-300 dark:border-slate-700'}`}>
                      <span className="text-xs font-bold uppercase text-slate-500 dark:text-slate-400">Comprometido</span>
                      <p className={`text-xl font-black ${summaryStats.percentCompromised > 90 ? 'text-yellow-600 dark:text-yellow-500' : 'text-slate-900 dark:text-white'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input 
                ...
                className="w-full border-b-2 border-slate-300 dark:border-slate-700 p-2 focus:border-slate-900 dark:focus:border-white outline-none bg-slate-50 dark:bg-slate-950 transition-colors font-medium text-slate-900 dark:text-white"
              />
            </div>
            <div>
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider border transition-all ${type === 'INCOME' ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 border-slate-900 dark:border-white' : 'bg-slate-100 dark:bg-slate-950 text-slate-500 border-slate-200 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-600'}`}
              >
                Receita
              </button>
              <button
                ...
                className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider border transition-all ${type === 'EXPENSE' ? 'bg-yellow-500 text-slate-900 border-yellow-500' : 'bg-slate-100 dark:bg-slate-950 text-slate-500 border-slate-200 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-600'}`}
              >
                Despesa
              </button>
              <button
                ...
                className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider border transition-all ${type === 'SAVING' ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-100 dark:bg-slate-950 text-slate-500 border-slate-200 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-600'}`}
              >
                Poupança
              </button>
            </div>
            ...
            {type === 'EXPENSE' && (
               <div className="bg-slate-100 dark:bg-slate-800 p-4 border border-slate-200 dark:border-slate-700 transition-colors">
                  <div className="flex items-center gap-2 mb-2">
                    <input 
                      ...
                      className="w-4 h-4 text-yellow-500 border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 rounded focus:ring-yellow-500"
                    />
                    <label htmlFor="isDebtPayment" className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase cursor-pointer">
                       Amortizar Dívida?
                    </label>
                  </div>
                  ...
                  {isDebtPayment && (
                    <div className="mt-2 animate-in fade-in duration-200">
                       <label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Selecione a Dívida</label>
                       <select
                         value={selectedDebtId}
                         onChange={(e) => setSelectedDebtId(e.target.value)}
                         className="w-full border border-slate-300 dark:border-slate-600 p-2 text-sm bg-white dark:bg-slate-950 text-slate-900 dark:text-white outline-none focus:border-slate-900 dark:focus:border-white transition-colors"
                       >
            ...
            {/* Recurrence Options */}
            <div className={`bg-slate-100 dark:bg-slate-800 p-4 border border-slate-200 dark:border-slate-700 mt-2 transition-all ${editingId ? 'border-l-4 border-l-yellow-500' : ''}`}>
              <div className="flex items-center gap-2 mb-3">
                <input 
                  ...
                  className="w-4 h-4 text-slate-900 dark:text-slate-900 border-slate-300 rounded focus:ring-slate-900"
                />
                <label htmlFor="isRecurring" className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase flex items-center gap-1 cursor-pointer">
              ...
              {isRecurring && (
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Repetir até qual mês?</label>
                  <input 
                    type="month" 
            ...
            <div className="flex gap-2 pt-4">
               {editingId && (
                   ...
                   type="button" 
                   onClick={cancelEdit}
                   className="flex-1 bg-slate-200 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 py-3 font-bold uppercase text-xs hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors"
                 >
                   Cancelar
                   </button>
               )}
        ...
        <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm transition-colors rounded-sm">
                <div className="bg-slate-100 dark:bg-slate-800 px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
                    <div>
                        <h4 className="font-bold text-slate-900 dark:text-white uppercase tracking-wide text-sm">Cluster: Vale</h4>
                        <p className="text-xs text-slate-500 dark:text-slate-400">Vencimentos: Dia 15 ao Dia 29</p>
                    </div>
                    <div className="text-right">
                        <p className="text-[10px] uppercase font-bold text-slate-500">Saldo Realizado</p>
                        <span className={`font-mono font-bold ${clusters.vale.total >= 0 ? 'text-slate-900 dark:text-white' : 'text-yellow-600 dark:text-yellow-500'}`}>R$ {clusters.vale.total.toLocaleString('pt-BR')}</span>
                    </div>
                </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm transition-colors rounded-sm">
                <div className="bg-slate-100 dark:bg-slate-800 px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
                    <div>
                        <h4 className="font-bold text-slate-900 dark:text-white uppercase tracking-wide text-sm">Cluster: Pagamento</h4>
                        <p className="text-xs text-slate-500 dark:text-slate-400">Vencimentos: Dia 30 ao Dia 14</p>
                    </div>
                    <div className="text-right">
                        <p className="text-[10px] uppercase font-bold text-slate-500">Saldo Realizado</p>
                        <span className={`font-mono font-bold ${clusters.pagamento.total >= 0 ? 'text-slate-900 dark:text-white' : 'text-yellow-600 dark:text-yellow-500'}`}>R$ {clusters.pagamento.total.toLocaleString('pt-BR')}</span>
                    </div>
                </div>
